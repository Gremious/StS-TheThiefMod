...
public class ThiefMod implements EditCardsSubscriber,
    ...
    private static void autoAddCards() throws URISyntaxException, IllegalAccessException, InstantiationException, NotFoundException, CannotCompileException, ClassNotFoundException {
        ...
        for (ClassInfo classInfo : foundClasses) {
            ...
            if(hasModCheck(classInfo.getClassName())) {
                ...
                try {
                    ...
                    System.out.println(classInfo.getClassName() + " could not be loaded because the mod is not present.");
                }
            }
        }
    }

    private static boolean hasModCheck(String s) {
        if(s.toLowerCase().contains("disciple") && !hasDisciple) {
            return false;
        }

        if(s.toLowerCase().contains("yohane") && !hasYohane) {
            return false;
        }

        if(s.toLowerCase().contains("blackruse") && !hasServant) {
            return false;
        }

        if(s.toLowerCase().contains("halation") && !hasHalation) {
            return false;
        }

        if(s.toLowerCase().contains("infinite") && !hasInfiniteSpire) {
            return false;
        }

        if(s.toLowerCase().contains("hubris") && !hasHubris) {
            return false;
        }

        if(s.toLowerCase().contains("replay") && !hasReplayTheSpire) {
            return false;
        }

        if(s.toLowerCase().contains("bard") && !hasBard) {
            return false;
        }

        if(s.toLowerCase().contains("conspire") && !hasConspire) {
            return false;
        }

        if(s.toLowerCase().contains("mystic") && !hasMysticMod) {
            return false;
        }
